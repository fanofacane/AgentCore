...
import com.sky.AgentCore.service.LlmDomainService;
import com.sky.AgentCore.service.ProductService;
...
public class UsageRecordBusinessInfoService {
    ...
    public Map<String, BusinessInfo> getBatchBusinessInfo(Set<String> productIds) {
        ...
        // 1. 批量查询商品信息
        Map<String, ProductEntity> productMap = productService.listByIds(productIds).stream()
        ...
        if (!modelIds.isEmpty()) {
            modelMap = llmDomainService.listByIds(modelIds).stream()
                    .collect(Collectors.toMap(ModelEntity::getId, model -> model));
        }

        Map<String, AgentEntity> agentMap = new HashMap<>();
        if (!agentIds.isEmpty()) {
            agentMap = agentMapper.selectByIds(agentIds).stream()
