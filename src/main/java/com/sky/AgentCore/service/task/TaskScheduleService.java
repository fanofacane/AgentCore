...
public class TaskScheduleService {
    ...
    public LocalDateTime calculateNextExecuteTime(ScheduledTaskEntity task, LocalDateTime currentTime) {
        ...
        switch (repeatType) {
            case NONE :
                ...
                if (executeTime != null  && executeTime.isAfter(currentTime)) {
                    return executeTime;
                }
                return null;
    ...
    public boolean shouldExecuteAt(ScheduledTaskEntity task, LocalDateTime checkTime) {
        ...
        if (!checkTime.isAfter(lastExecuteTime.plusMinutes(TIME_TOLERANCE_MINUTES))) {
            return false;
        }
        ...
        switch (repeatType) {
            case NONE :
                // 一次性任务，只在首次执行时间执行
                return  checkTime.isAfter(executeTime.minusMinutes(TIME_TOLERANCE_MINUTES))
