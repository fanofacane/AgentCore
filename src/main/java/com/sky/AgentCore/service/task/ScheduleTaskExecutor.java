package com.sky.AgentCore.service.task;


import com.sky.AgentCore.dto.scheduledtask.ScheduledTaskEntity;
import com.sky.AgentCore.dto.scheduledtask.event.ScheduledTaskExecuteEvent;
import com.sky.AgentCore.enums.RepeatType;
import com.sky.AgentCore.service.task.domain.ScheduledTaskDomainService;
...
public class ScheduleTaskExecutor {
    ...
    @Autowired
    private ScheduledTaskDomainService scheduledTaskDomainService;
    @Autowired
    ...
    public boolean canExecute(ScheduledTaskEntity task) {
        ...
        if (!taskScheduleService.shouldExecuteAt(task, now)) {
            logger.info("任务当前时间不应该执行: taskId={}, currentTime={}", task.getId(), now);
            return false;
        }
